...
class QrScannerViewController: UIViewController {
    ...
    @IBAction func LoginbtnClick(_ sender: UIButton) {
        if scandata==nil{
            ...
            checkLogin(un:scandata!)
            
        }
        
    }
    func changetextbtn(text:String){
        DispatchQueue.main.async {
            self.scanorloginbtn.setTitle(text, for: .normal)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
             ...
             guard let captureDevice = AVCaptureDevice.default(for: AVMediaType.video) else {
                ...
                changetextbtn(text: "Fail Camera")
                return
             }
         ...
         func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
             ...
             if supportedCodeTypes.contains(metadataObj.type) {
                 ...
                 if metadataObj.stringValue != nil  {
                    view.bringSubviewToFront(scanorloginbtn)
                     //launchApp(decodedURL: metadataObj.stringValue!)

                    print(metadataObj.stringValue as Any)
                    scandata=metadataObj.stringValue
//                    dismiss(animated: true, completion: nil)
                    changetextbtn(text: "Login")
        ...
        override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
            if segue.destination is TestViewController{
                ...
                ComfirmPasswordViewController.userdata=userdata
                Tab1ViewController.user_data=userdata
            }
        }
                   ...
                   if let json=try? JSONSerialization.jsonObject(with: Data!, options:.mutableContainers)as? NSMutableDictionary{
                       ...
                       if json["response"]as!String=="customer"{
                           DispatchQueue.main.sync {
                            ...
                            self.performSegue(withIdentifier: "comfirmiden", sender: self)
                           }
        ...
        func ShowIndicator(boo:Bool){
            DispatchQueue.main.async {
                if boo{
                    self.scanorloginbtn.isEnabled=false
                    self.loading.alpha=1
                    self.loading.startAnimating()
                }
                    else {
                    self.scanorloginbtn.isEnabled=true
